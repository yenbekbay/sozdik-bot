...
import env from '../env';
import type {Chat, ParseMode, Message, InlineQueryResult} from './types';
...
export type AnswerInlineQueryFn = (
  config: AnswerInlineQueryConfig,
) => Promise<?JSON>;
...
const apiRequest = (method: ApiMethod, params: Object): Promise<JSON> =>
  request.post({
...
const telegramBotApi = (logger: Logger) => ({
  sendMessage: ({chat, text, ...options}: SendMessageConfig) =>
  ...
  sendChatAction: ({chat, action}: SendChatActionConfig) =>
  ...
  answerInlineQuery: ({inlineQueryId, results}: AnswerInlineQueryConfig) =>

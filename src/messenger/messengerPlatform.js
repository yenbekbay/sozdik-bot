...
export type SendTextMessageFn =
  (config: SendTextMessageConfig) => Promise<?JSON>;
export type SendSenderActionFn =
  (config: SendSenderActionConfig) => Promise<?JSON>;
...
const messengerPlatform = (logger: Logger) => ({
  sendTextMessage: (
    { recipientId, text }: SendTextMessageConfig,
  ...
  sendSenderAction: (
    { recipientId, action }: SendSenderActionConfig,
